...
/*&& riesling !== '' && gewur !== '' && viognier !== '' && chenin !== '' && chardonnay !== '' && sauvignon !== ''
  ...
  const checkFormAndValid = (action: string) => {
    ...
    if (action === 'add') {
      ...
      let editJson: any = {
        ...
        cepage: cepagesObj !== '' ? cepagesObj : null,
        label_img: '',
        product_img: '',
        buy_price_ht: buy_price_ht !== '' ? buy_price_ht : null,
        sell_price_ht: sell_price_ht !== '' ? sell_price_ht : null,
        quantity: quantity !== '' ? quantity : null,
        hold: 0,
        active: isEnabled,
      }

      for (const elm in editJson) {
        if (editJson[elm] === null) {
          delete editJson[elm];
        }
      }

      fetch('http://146.59.156.251:3000/products/update/' + item[0]._id, {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
        ...
        .then(json => {
          navigation.navigate('Products', {wasUpdated: true});
        })
    }
        ...
        <View style={styles.wrapper}>
          <View style={styles.block}>
            ...
            <TextInput
              ...
              value={ brand_name }
              placeholder={edit ? item[0].brand_name : ''}
            />
          </View>
          <View style={styles.block}>
          ...
          <View style={styles.block}>
            { edit ? (
              <Button
                ...
                onPress={() => {
                  checkFormAndValid('edit')
                }}
              />
            ) : (
              <Button
                title={'Ajouter le produit'}
                onPress={() => checkFormAndValid('add') }
              />
            )}
          </View>
        </View>
